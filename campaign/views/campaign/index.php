<div class="layout_rightmain">
            ...
            <div class="r-middle">
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;
                <select name="theme" id="theme" class="select select-long">
            		<option>请选择</option>
            		<?php foreach($themeArr as $theme):?>
            		<option value="<?php echo $theme['id'];?>"><?php echo $theme['title'];?></option>
            		<?php endforeach;?>
            	</select>
            	<a href="javascript:void(0);" onclick="setTheme();" class="btn" />设置主题</a>
            </div>
            <div class="r-bottom">
                <div class="list-table">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                      	<th><span onclick="selectAll();" style="cursor:pointer;">全选</span></th>
                        <th width="10%"><span>标题</span></th>
                        <th width="8%"><span>目的地</span></th>
                		<th width="8%"><span>来源</span></th>
                		<th width="12%"><span>添加时间</span></th>
                		<th width="27%"><span>其余信息</span></th>
                        <th width="16%"><span>置顶</span></th>
                        <th width="14%"><span>操作</span></th>
              		</tr>
              <?php foreach ($list as $camp):?>
              <tr>
              	<td><input style="width:30px;" type="checkbox" value="<?php echo $camp['id'];?>" name="themeSet"></td>
...
function selectAll(){
	var boolVar = $("[name=themeSet]:checkbox").length == $("[name=themeSet]:checked").length;
	$("[name=themeSet]:checkbox").attr("checked", !boolVar);	
}
function setTheme(){
	var str = "";
	$("[name=themeSet]:checked").each(function(){
		str += "," + this.value;
	});	
	var themeId = $("#theme").val();
	$.ajax({
	    url:'/campaign/theme-set',
	    type:'POST', 
	    async:true,    //或false,是否异步
	    data:{
	        themeId : themeId,
