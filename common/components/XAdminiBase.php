...
//            $session->open();
//        }
        $this->_admini = AuthorizeVerify::getUser('userInfo');
//        if (empty($this->_admini['userId']))
//            $this->redirect(array ('public/login' ));
//        //栏目
        $this->_catalog = Catalog::find()->where(['status_is'=>'Y'])->all();
        
//        dprint($this->_catalog);
        //系统配置
//        $this->_conf = self::_config();
    ...
    {
//        dprint($this->id . '_' . $this->action->id);
        $actionFormat = empty($action) ? strtolower($this->id . '_' . $this->action->id) : strtolower($action);
        if (empty($this->_admini['super'])) {
//            dprint($this->_admini);
            $aclDb = AdminGroup::findOne($this->_admini['groupId']);
//            dprint($aclDb);
            try {
                dprint(strtolower($aclDb->acl),$params['append']);
